...
def derive_baseline_features(df):
    df = df[
        ...
        baseline_feature_target + ['y_drop', 'target', 'train']
...
# feature selection
def select_features(X, y, X_test, y_test):
    ...
    return X_train_fs, y, features_df_new, X_test_fs, y_test
...
def run_regression(df:pd.DataFrame, mode:str, feature_selection_bool:bool, across_domain = False):
    ...
    y_train = df_train[['y_drop']]
    y_test = df_test[['y_drop']]
    ...
    X_train = df_train.drop(['y_drop', 'train'], axis=1)
    X_test = df_test.drop(['y_drop', 'train'], axis=1)
    # Extract text features
    cats_train = X_train.select_dtypes(exclude=np.number).columns.tolist()
    cats_test = X_test.select_dtypes(exclude=np.number).columns.tolist()
    ...
    if feature_selection_bool:
        X_train, y_train, selected_features, X_test, y_test = select_features(X_train, y_train, X_test, y_test)
...
if __name__ == '__main__':
    ...
    cache_directory = "training_features/2024-12-14_11-13-51"
    if cache:
        directory = cache_directory
    file_name= "training_Features_10_domains.xlsx"
