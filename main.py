...
domain_specific_features = [
    ...
    "target_shannon_entropy",
]
features_to_normalize = {
    "source": [
        "source_bert_precision",
        "source_bert_recall",
        "source_bert_f1",
        "source_vocab_overlap",
        "source_Relevance",
...
def xgboost(X_train, X_test, y_train, y_test):
    ...
    model = xgb.train(
        params=params, dtrain=dtrain_reg, num_boost_round=n, evals=evals, verbose_eval=2
    )
    ...
    scores = {
        "xgboost-mse": float(round(mse, 3)),
        "xgboost-mae": float(round(mae, 3)),
        "xgboost-rmse": float(round(rmse, 3)),
        "xgboost-r2": float(round(r2, 3)),
    }
...
def ridge_regression(X_train, X_test, y_train, y_test):
    ...
    ridge_reg = RidgeCV().fit(
        X_train, y_train
    )  # You can change the alpha parameter to add more or less regularization
    ...
    scores = {
        "ridge-mse": float(round(mse, 3)),
        "ridge-mae": float(round(mae, 3)),
        "ridge-rmse": float(round(rmse, 3)),
        "ridge-r2": float(round(r2, 3)),
    }
    # print(scores)
    scores.update(kfold_scores)
    return scores
...
def lasso_regression(X_train, X_test, y_train, y_test):
    ...
    # Instantiate the Ridge Regression model
    lasso_reg = linear_model.LassoCV().fit(
        X_train, y_train
    )  # You can change the alpha parameter to add more or less regularization
    ...
    scores = {
        "lasso-mse": float(round(mse, 3)),
        "lasso-mae": float(round(mae, 3)),
        "lasso-rmse": float(round(rmse, 3)),
        "lasso-r2": float(round(r2, 3)),
    }
    # print (scores)
    scores.update(kfold_scores)
    return scores
...
def linear_regression(X_train, X_test, y_train, y_test):
    ...
    kf = KFold(n_splits=3, shuffle=True, random_state=42)
    kfold_scores = cross_validation(
        reg, np.vstack((X_train, X_test)), np.vstack((y_train, y_test)), kf, model=""
    )
    # Train the model
...
def derive_baseline_features_red(df):
    df = df[
        domain_specific_features + baseline_feature_source + baseline_feature_target
    ]
...
def derive_baseline_features(df):
    ...
    weighted_y_target = weighted_average_list(
        (df[baseline_feature_target]).values, feature_weight
    )
    weighted_y_source = weighted_average_list(
        df[baseline_feature_source].values, feature_weight
    )
...
def normalize_features(df):
    def norm(df, features_to_normalize, update_y=None, a=None):
        for feature in features_to_normalize:
            if feature in df.columns:
                ...
                old_range_key = [
                    key for key in normalize_range.keys() if key in feature
                ][0]
        ...
        if a is not None:
            ...
            df[update_y] = weighted_average_list(
                (df[weighted_y_col]).values, feature_weight
            )
    ...
    # normalize target
    df = norm(
        df, features_to_normalize["target"], update_y="weighted_y_target", a="target_"
    )
    # normalize source
    df = norm(
        df, features_to_normalize["source"], update_y="weighted_y_source", a="source_"
    )
...
def select_features(X, y):
    ...
    if k < len(feature_names) - 1:
        ...
        print(
            f"Automatic Reduced features from {len(feature_names)} to {len(X_train_fs[0])}"
        )
    else:
        X_train_fs = X
        features_df_new = []
...
def cross_validation(reg_model, X, y, cv, model=""):
    ...
    scores = cross_val_score(reg_model, X, y, scoring="neg_mean_squared_error", cv=cv)
    mse = np.sqrt(-scores)
    mse = mse.mean()

    scores = cross_val_score(reg_model, X, y, scoring="neg_mean_absolute_error", cv=cv)
    mae = -scores
    mae = mae.mean()
    ...
    scores = cross_val_score(reg_model, X, y, scoring="r2", cv=cv)
    r2 = scores
    r2 = r2.mean()

    scores = {
        model + "-k-fold-mse": float(round(mse, 3)),
        model + "-k-fold-mae": float(round(mae, 3)),
        model + "-k-fold-rmse": float(round(rmse, 3)),
...
def run_regression(
    df: pd.DataFrame, mode: str, feature_selection_bool: bool, across_domain=False
):

    if mode == "baseline-raw" or mode == "baseline-norm":
        print(mode)
        features_to_drop = baseline_feature_target + [
            "weighted_y_target",
            "weighted_y_source",
            "target",
        ...
        features_to_drop = [
            ...
            "vocab-overlap",
            "source_shannon_entropy",
        ] + list(reduced_features_target.keys())
...
def reduce_features_all(df):
    ...
    reduced_features_target_values = [
        v for value in reduced_features_target.values() for v in value
    ]
    reduced_features_source_values = [
        v for value in reduced_features_source.values() for v in value
    ]
    ...
    df = df.drop(
        reduced_features_target_values + reduced_features_source_values, axis=1
    )
    num_features = len(reduced_features_target.keys())
    feature_weight = [1 / num_features] * num_features

    weighted_y_target = weighted_average_list(
        (df[list(reduced_features_target.keys())]).values, feature_weight
    )
    weighted_y_source = weighted_average_list(
        df[list(reduced_features_source.keys())].values, feature_weight
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--da_type", type=str, default="in-domain-adapt")
    parser.add_argument(
        "--domain",
        dest="domains",
        action="append",
    ...
    if cache and os.path.isfile(file_name):
        ...
        all_features = construct_training_corpus(
    ...
    for feat_selection in sklearn_feature_selection:
        for n in range(minumum_domains, total_domains + 1, 1):
            ...
            scores_baseline_norm, selected_feat = run_regression(
                features_baseline_norm,
                mode="baseline-norm",
                feature_selection_bool=feat_selection,
            ...
            scores_all_norm, selected_feat = run_regression(
                features_norm,
                mode="all-norm",
                feature_selection_bool=feat_selection,
                across_domain=across_domain,
            ...
            pd_scores = pd.DataFrame.from_records(
                [scores_baseline_norm, scores_all_norm]
            )
